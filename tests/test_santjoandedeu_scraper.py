...
def test_extract_items_rejects_non_json_listing():
    ...
    assert exc_info.value.site_id == "santjoandedeu"
    assert "no devolvió JSON válido" in exc_info.value.detail


def test_extract_items_from_fallback_ajax_html():
    scraper = SantJoanDeDeuScraper()
    soup = BeautifulSoup(
        """
        <article class="entry">
            <a class="link" href="https://sjd.es/sjd-barcelona-participa-cirugia-fetal-europa/"></a>
            <div class="entry-content">
                <h3 class="entry-title">
                    SJD Barcelona participa en una cirugía fetal pionera en Europa
                </h3>
                <span>29 mayo 2026</span>
                <p class="resum">Resumen de la noticia...</p>
            </div>
        </article>
        """,
        "lxml",
    ...
    items = list(scraper._extract_items_from_fallback_html(soup))

    assert len(items) == 1
    assert items[0].title == "SJD Barcelona participa en una cirugía fetal pionera en Europa"
    assert items[0].url == "https://sjd.es/sjd-barcelona-participa-cirugia-fetal-europa"
    assert items[0].published_at == datetime(2026, 5, 29, tzinfo=timezone.utc)
    assert items[0].metadata["fallback"] == "https://sjd.es/noticias/"
